...
export const Names = () => { 
    ...
    return (
        <>
            <group>
                {nameinfo.map((info) => (
                    <Html key={info.id} painted distanceFactor={5}  center position={info.position}> 
                    <button className="text_names" onClick={(e) => console.log(info.name)}><h1>{info.text}</h1></button>
                    </Html>
                ))}
            </group>
        </>
